...
\end{align}
No identification with the derivative of a time-dependent coherent-state
label is made here.  For each model, the continuum Hamiltonian flow is
derived independently and used to identify the continuum limit of the
left-hand side with the time derivative of the classical spatial Lax matrix.
The two products on the right-hand side cannot be simplified by applying the
lower symbol to each factor separately, because the two factors in each
product act on a common physical site.  To keep track of this point without
introducing any model-dependent formula, define the two auxiliary-space
matrices
\begin{align}
\mathcal D^{(+)}_{a,n}(u)
:={}&
...
\end{align}
The right-hand side of the lattice equation therefore starts at order

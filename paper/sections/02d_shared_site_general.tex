...
$P_+:=(\Id+P)/2$ and the condition
\begin{equation}
P_+h^{(1)}P_+=cP_+
\label{eq:common-triplet-condition}
\end{equation}
for a field-independent scalar $c$.  This is a sufficient condition for the
common identity below and holds for both chains studied here.

For the two models, Eq.~\eqref{eq:common-triplet-condition} also follows from
the Sutherland relation rather than being an independent model assumption.
To see this, expand the first spatial coefficient in the physical Pauli basis,
\begin{equation}
...
\end{equation}
The tensor product of two spin-$\tfrac12$ representations decomposes into one
spin triplet and one spin singlet, each with multiplicity one.  Hence an
operator commuting with all three total-spin generators is scalar on each
sector.  The exchange-odd part of $h^{(1)}$ has vanishing triplet-to-triplet
matrix elements, so Eq.~\eqref{eq:common-triplet-condition} follows.  For
both Class 5 and Class 6, the spin dependence has rank three for
$\lambda\ne0$; for example,
\begin{equation}
...
\end{equation}
For a rank-deficient spatial coefficient the implication need not hold, so we
retain Eq.~\eqref{eq:common-triplet-condition} as the stated sufficient
condition of the general formula.
...
\end{align}
We now suppress the model label $m$ on the coefficient functions
...
The order-$\epsilon^2$ parts of the two overlapping products have equal
coincident-spin values,
\begin{equation}
...
\end{equation}
The terms containing $Y$ are required in the intermediate expansion but
cancel after all contributions in Eq.~\eqref{eq:common-time-coefficients} are
combined.
...
For spin $\tfrac12$, the same Pauli expansion
Eq.~\eqref{eq:common-X-rank-expansion} gives
$U=\mathsf X_0+\mathsf X_iS^i$.  The second moment in
Eq.~\eqref{eq:common-Xi-definition} is therefore
\begin{equation}
\Xi

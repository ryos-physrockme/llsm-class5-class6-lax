...
\end{align}
For the overlap differences defined in
Eqs.~\eqref{eq:Rplus-hat-definition} and \eqref{eq:Rminus-hat-definition},
write $\mathcal D^{(\pm)}_2$ and $\mathcal D^{(\pm)}_3$ for the coefficients
of $\epsilon^2$ and $\epsilon^3$, with the left and right spin arguments
shown explicitly when needed.  We also write $\langle Q\rangle$ for the
lower symbol of a two-physical-site operator $Q$ evaluated with the same

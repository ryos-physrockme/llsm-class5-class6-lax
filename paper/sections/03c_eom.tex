...
$\kappa_5=\epsilon\alpha_5$, the lower symbol of the Class 5 bond
Hamiltonian has the long-wavelength expansion
\begin{equation}
 h_{5;n,n+1}^{\downarrow}
 =2+\epsilon^2\mathcal H_5(x_n,t)+\mathcal O(\epsilon^3),
\label{eq:class5-h-lower-expansion}
\end{equation}
where the Hamiltonian density at $(x,t)$ is
\begin{equation}
\mathcal H_5(x,t)
=
...
\end{equation}
where $\delta(x-y)$ is the Dirac delta function and the common time argument
is suppressed.  Varying the Cartesian spin components before imposing the
unit-spin constraint, and integrating by parts with periodic variations,
gives $\delta H^{\rm cl}_5/\delta\boldsymbol S
=\partial_x^2\boldsymbol S+\alpha_5M_5\partial_x\boldsymbol S$.
...
The same flow follows directly from the finite-lattice Heisenberg derivative.
Let $\boldsymbol\sigma_n=(\sigma_n^1,\sigma_n^2,\sigma_n^3)^{\mathsf T}$,
where $\sigma_n^i$ is the Pauli matrix $\sigma^i$ acting at site $n$.
The two bonds adjacent to that site give
\begin{align}
\frac{1}{\epsilon^2}
...
\nonumber\\
&\hspace{6em}
+\frac{\alpha_5}{2\epsilon}M_5
\left(\boldsymbol S_{n+1}-\boldsymbol S_{n-1}\right)
\Biggr].
\label{eq:class5-finite-lattice-spin-flow}
\end{align}
Its smooth-field limit is Eq.~\eqref{eq:class5-vector-eom}.
Applying the same commutator to the normalized lattice Lax operator gives
\begin{equation}
\left(
...
\frac{1}{4\lambda}
\begin{pmatrix}
E^{(5)}_3+2\alpha_5\lambda E^{(5)}_+
&
E^{(5)}_- -2\alpha_5\lambda E^{(5)}_3\\
E^{(5)}_+
&
-E^{(5)}_3
\end{pmatrix}.
...
\begin{align}
E^{(5)}_+&=4\lambda\bigl(F^{(5)}_{tx}\bigr)_{21},
\qquad
E^{(5)}_3=-4\lambda\bigl(F^{(5)}_{tx}\bigr)_{22},
\nonumber\\
E^{(5)}_-&=4\lambda\left(
...
Ref.~\cite{deLeeuwFontanellaNietoGarcia2026}, denote its spin field, coupling,
and time by $\boldsymbol S_{\rm ref}$, $\alpha_{\rm ref}$, and
$t_{\rm ref}$.  The convention dictionary is
\begin{equation}
\begin{gathered}
t_{\rm ref}=2t,
\qquad \alpha_{\rm ref}=-\alpha_5,\\
\boldsymbol S_{\rm ref}(x,t_{\rm ref})
=\operatorname{diag}(1,-1,1)\,
\boldsymbol S(x,t_{\rm ref}/2).
\end{gathered}
\label{eq:class5-reference-convention-change}
\end{equation}
With these identifications, Eq.~\eqref{eq:class5-vector-eom} takes the form of

...
$\widehat L_6$ vanishes.  The coefficients in
Eqs.~\eqref{eq:class6-A1} and \eqref{eq:class6-A2} nevertheless depend on
$\alpha_6$ through $L_6^{(1)}$ and $L_6^{(2)}$.
...
$\epsilon^4$.  Thus the displayed coefficients suffice for the continuum
source, but the products involving $L_6^{(2)}$ must be retained.

At coincident spins, the order-$\epsilon^2$ terms cancel between
$\widehat{\mathcal D}^{(+)}_{6;a,n}$ and
$\widehat{\mathcal D}^{(-)}_{6;a,n}$.  Expanding their spin arguments to
first order, and evaluating the order-$\epsilon^3$ products at coincident
spins, yields
\begin{equation}
\mathcal C_6
...
\end{equation}
Here $\Xi_6$ is the difference between the lower symbol of the square of
...
\end{equation}
The calculation of Eq.~\eqref{eq:class6-shared-site-correction} uses only
the unit-spin constraint and its spatial derivative.  It does not assume
...
For an explicit component form, write $S_x^a:=\partial_x S^a$ for
$a\in\{+,-,3\}$.  The entries of $V_6$ are
\begin{align}
(V_6)_{11}
...
\end{equation}
For example, at a spatially uniform configuration
$\boldsymbol S=(0,0,1)^{\mathsf T}$,
\begin{equation}
U_6=\begin{pmatrix}
...
\end{equation}
This is an instantaneous spatial configuration, not an assumption of
stationary time evolution.  Its source is nonzero for $\alpha_6\ne0$.
...
Equations~\eqref{eq:class6-Vdown-direct} and
\eqref{eq:class6-DeltaV-absorbs-C} establish the time matrix and the local
absorption of the contribution from overlapping products.  In
Section~\ref{sec:class6-eom} we derive the Hamiltonian equations

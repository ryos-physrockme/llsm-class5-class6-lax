...
Eq.~\eqref{eq:finite-lattice-time-Lax}.  Adding the same scalar multiple of
the identity to every time Lax operator leaves the lattice zero-curvature
equation unchanged.  We use the representative
\begin{equation}
\widehat A_{5;a,n}
:=
...
parameter $\kappa_5$.  After imposing the continuum scalings
...
\nonumber\\
&\quad-\ii[h^{(1)}_{n-1,n},L^{(1)}_{5;a,n}]
...
\begin{equation}
\bigl(A^{(1)}_{5;a,n}\bigr)^\downarrow
=
-2
...
\end{equation}
The Cartesian derivatives act on Eq.~\eqref{eq:class5-spatial-Lax}, with
$S^\pm=S^1\pm\ii S^2$, before imposing the unit-spin constraint.  They are
independent of the spin because $U_5$ is affine in its components.  Using
\begin{equation}
\boldsymbol S_{n-1}
=
...
\end{equation}
The term $A^{(2)}_{5;a,n}$ already carries the prefactor $\epsilon^2$ in
...
\begin{equation}
\left.\bigl(A^{(2)}_{5;a,n}\bigr)^\downarrow\right|_{\boldsymbol S_{n-1}=\boldsymbol S_n=\boldsymbol S}
=
...
The remaining contribution comes from the overlapping operator products in
the discrete zero-curvature equation.  For Class 5, denote the differences
in Eqs.~\eqref{eq:Rplus-hat-definition} and
\eqref{eq:Rminus-hat-definition} by
$\widehat{\mathcal D}^{(\pm)}_{5;a,n}$.  The quadratic coefficients in
Eqs.~\eqref{eq:overlap-plus-coefficients} and
\eqref{eq:overlap-minus-coefficients} contain the products of
$A_5^{(1)}$ with $L_5^{(1)}$; the cubic coefficients contain the
products of $A_5^{(2)}$ with $L_5^{(1)}$.  Evaluating the shared-site
Pauli products with Eq.~\eqref{eq:single-site-pauli-symbol-product}
gives
\begin{align}
\mathcal D^{(+)}_{5,2}(\boldsymbol S,\boldsymbol S)
&=\mathcal D^{(-)}_{5,2}(\boldsymbol S,\boldsymbol S)
=2\ii\left(\frac{\Id_{\mathcal V_a}}{4\lambda^2}-U_5^2\right),
\label{eq:class5-coincident-quadratic-overlap}\\
\partial_x S^i
\left.
\left(
\frac{\partial\mathcal D^{(+)}_{5,2}}{\partial S_R^i}
+\frac{\partial\mathcal D^{(-)}_{5,2}}{\partial S_L^i}
\right)\right|_{\boldsymbol S_L=\boldsymbol S_R=\boldsymbol S}
&=-2\ii\partial_x(U_5^2),
\label{eq:class5-quadratic-overlap-taylor}\\
\mathcal D^{(+)}_{5,3}(\boldsymbol S,\boldsymbol S)
-\mathcal D^{(-)}_{5,3}(\boldsymbol S,\boldsymbol S)
&=0.
\label{eq:class5-coincident-cubic-overlap}
\end{align}
The first and third identities use the unit-spin constraint.  The
second also uses $\boldsymbol S^{\mathsf T}\partial_x\boldsymbol S=0$.
Thus the surviving contribution comes entirely from the first line of
Eq.~\eqref{eq:overlap-source-taylor}.  In terms of the spatial operator,
\begin{align}
&\widehat{\mathcal D}^{(+)}_{5;a,n}
-\widehat{\mathcal D}^{(-)}_{5;a,n}
\nonumber\\
&\qquad=
...
\end{equation}
We write $S_x^a:=\partial_x S^a$ for $a\in\{+,-,3\}$.  Substituting
Eq.~\eqref{eq:class5-spatial-Lax} and using the unit-spin constraint gives
the matrix elements
\begin{align}
(V_5)_{11}
={}&\frac{\ii}{4\lambda^2}
\Bigl[
S^3+\alpha_5\lambda S^+(1+S^3)+2\alpha_5^2\lambda^2(S^+)^2
\nonumber\\
&\hspace{3.2em}
+\lambda(S^-S_x^+-S^+S_x^-)
+4\alpha_5\lambda^2(S^+S_x^3-S^3S_x^+)
\Bigr],
\label{eq:class5-time-Lax-11}\\
(V_5)_{12}
={}&\frac{\ii}{4\lambda^2}
\Bigl[
S^- -\alpha_5\lambda(1+S^3)^2-2\alpha_5^2\lambda^2S^+(1+S^3)
\nonumber\\
&\hspace{3.2em}
+2\lambda(S^3S_x^--S^-S_x^3)
+2\alpha_5\lambda^2(S^+S_x^--S^-S_x^+)
\Bigr],
\label{eq:class5-time-Lax-12}\\
(V_5)_{21}
={}&\frac{\ii}{4\lambda^2}
\Bigl[
S^+ +\alpha_5\lambda(S^+)^2

...
\end{equation}
The continuum limit also rescales the Hamiltonian, time coordinate, and
deformation parameters.  The common time scaling is fixed in
Section~\ref{sec:lattice-to-continuum-zc}; the coupling scalings are
determined in the two model sections.
...
The product of two Pauli matrices on the same site satisfies
$\sigma^i\sigma^j=\delta_{ij}\Id_2+\ii\varepsilon_{ijk}\sigma^k$,
where $\delta_{ij}$ is the Kronecker delta and $\varepsilon_{ijk}$
is completely antisymmetric with $\varepsilon_{123}=1$.  Thus
\begin{equation}
(\sigma^i\sigma^j)^\downarrow
-(\sigma^i)^\downarrow(\sigma^j)^\downarrow
=\delta_{ij}-S^iS^j+\ii\varepsilon_{ijk}S^k.
\label{eq:single-site-pauli-symbol-product}
\end{equation}
Together with factorization between distinct physical sites, this
identity evaluates all overlapping operator products used below.

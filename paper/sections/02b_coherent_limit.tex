...
\end{equation}
where the fields on the right-hand side are evaluated at $(x_n,t)$.  The unit
length constraint in \eqref{eq:continuum-spin-field} implies the useful
...
they are not fixed by the coherent-state construction alone.

The lower symbol of a single local operator is therefore straightforward.
The next step is more delicate because the discrete zero-curvature equation
\eqref{eq:discrete-zero-curvature} contains products of operators with
overlapping physical support.  We now determine how such products enter the

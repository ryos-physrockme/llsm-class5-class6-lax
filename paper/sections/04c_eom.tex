...
Expanding the bond lower symbol in
Eq.~\eqref{eq:class6-bond-lower-symbol} with
$\kappa_6=\epsilon^2\alpha_6$ gives
\begin{equation}
h_6^\downarrow
\bigl(\boldsymbol S(x,t),\boldsymbol S(x+\epsilon,t)\bigr)
=2+\epsilon^2\mathcal H_6(x,t)+\mathcal O(\epsilon^3),
\label{eq:class6-h-lower-expansion}
\end{equation}
where the continuum Hamiltonian density is
\begin{equation}
\mathcal H_6
...
continuum Hamiltonian on the periodic interval of length $\ell=N\epsilon$
is
\begin{align}
H^{\rm cl}_6
&:=\lim_{\epsilon\to0}
...
We use the spin Poisson bracket with the same normalization as in
...
is the Dirac delta function.  Integration by parts with periodic
variations gives
...
finite lattice spacing.  Let
$\boldsymbol\sigma_n=(\sigma_n^1,\sigma_n^2,\sigma_n^3)^{\mathsf T}$.
Evaluating the Heisenberg commutator on a product of coherent states
at a fixed lattice time gives
\begin{align}
\frac{1}{\epsilon^2}
...
\end{equation}
and write its transverse components as
$E^{(6)}_\pm:=E^{(6)}_1\pm\ii E^{(6)}_2$.
